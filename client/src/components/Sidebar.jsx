...
const navItems = [
    ...
    { to: '/ai/blog-titles', label: 'Blog Articles', Icon: Hash },
...
const Sidebar = ({ sidebar, setSidebar }) => {
    ...
    return (
        <div className={`w-60 bg-white border-r border-gray-200 flex flex-col justify-between items-center
            max-sm:absolute top-14 bottom-0 ${sidebar ? 'translate-x-9' : 'max-sm:-translate-x-full'} transition-all duration-300 ease-in-out`}>
                ...
                <div className='px-6 mt-5 text-sm text-gray-600 font-medium'>
                    {navItems.map(({ to, label, Icon }) => (
                        <NavLink key={to} to={to} end={to === '/ai'} onClick={() =>
                            setSidebar(false)} className={({ isActive }) => `px-3.5 py-2.5 flex items-center
                            gap-3 rounded ${isActive ? 'bg-gradient-to-r from-[#3C81F6] to-[#9234EA] text-white' : ''}`}>
                            {
                                ({ isActive }) => (
                                    <>
